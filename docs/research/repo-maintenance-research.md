...
- **Stable section names as a contract.** WRITING-GUIDE.md explicitly calls these out: `## Foundations` in FOUNDATIONS.md, `## Quick Start` in README.md, env-var tables under known headings. Adding sections is safe; renaming or moving them is a breaking change requiring coordinated updates to consumers (agentkit, plus dockit's own `sync` and `check`). This is the docs-as-tooling-contract pattern from the docs-linting community.

  > **Note (2026-08-01):** this list originally also named `feedback-loop` and `onboard`. Both were removed from repokit when the plugin tightened to dockit + agentkit + hub; the contract itself is unchanged, it just has fewer consumers.
